...
public class CustomizeExceptionHandler extends ResponseEntityExceptionHandler {
  ...
  public ResponseEntity<Object> handleInvalidRequest(InvalidRequestException e) {
    return ResponseEntity.status(UNPROCESSABLE_ENTITY)
        .body(ErrorResource.body(e.getField(), e.getMessage()));
  }
  ...
  protected ResponseEntity<Object> handleMethodArgumentNotValid(
      ...
      HttpStatus status,
      WebRequest request) {
    Map<String, Map<String, List<String>>> message =
        e.getBindingResult().getFieldErrors().stream()
            .findFirst()
            .map(fe -> ErrorResource.body(fe.getField(), fe.getDefaultMessage()))
            .orElse(ErrorResource.body("invalid request body"));

package io.spring.infrastructure.extraction.comment;

import io.spring.application.CursorPageParameter;
import io.spring.application.comment.CommentQueryPort;
import io.spring.application.data.CommentData;
import io.spring.infrastructure.extraction.ExtractionProperties;
import io.spring.infrastructure.extraction.ExtractionProperties.DomainRoute;
import io.spring.infrastructure.extraction.ReadAfterWriteMarker;
import io.spring.infrastructure.extraction.ShadowComparator;
import io.spring.infrastructure.mybatis.readservice.CommentReadService;
import java.util.ArrayList;
import java.util.List;
import java.util.function.Supplier;
import org.joda.time.DateTime;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.context.annotation.Primary;
import org.springframework.stereotype.Service;

/**
 * Picks the monolith or the remote {@link CommentQueryPort} on every call according to {@code
 * extraction.comment.*}. In {@code shadow} mode the monolith answer is returned and the remote one
 * is compared in the background; in {@code extracted} mode a remote failure is handled per {@code
 * fallback}. While the monolith is still authoritative for writes, a read that follows a write in
 * the same request is always served locally so the response reflects what was just written.
 */
@Primary
@Service
public class RoutingCommentQueryPort implements CommentQueryPort {
  static final String DOMAIN = "comment";
  private static final Logger log = LoggerFactory.getLogger(RoutingCommentQueryPort.class);

  private final CommentReadService monolith;
  private final RemoteCommentQueryAdapter remote;
  private final ExtractionProperties properties;
  private final ShadowComparator shadow;
  private final ReadAfterWriteMarker readAfterWrite;

  public RoutingCommentQueryPort(
      CommentReadService monolith,
      RemoteCommentQueryAdapter remote,
      ExtractionProperties properties,
      ShadowComparator shadow,
      ReadAfterWriteMarker readAfterWrite) {
    this.monolith = monolith;
    this.remote = remote;
    this.properties = properties;
    this.shadow = shadow;
    this.readAfterWrite = readAfterWrite;
  }

  @Override
  public CommentData findById(String id) {
    return route("findById", () -> monolith.findById(id), () -> remote.findById(id), () -> null);
  }

  @Override
  public List<CommentData> findByArticleId(String articleId) {
    return route(
        "findByArticleId",
        () -> monolith.findByArticleId(articleId),
        () -> remote.findByArticleId(articleId),
        ArrayList::new);
  }

  @Override
  public List<CommentData> findByArticleIdWithCursor(
      String articleId, CursorPageParameter<DateTime> page) {
    return route(
        "findByArticleIdWithCursor",
        () -> monolith.findByArticleIdWithCursor(articleId, page),
        () -> remote.findByArticleIdWithCursor(articleId, page),
        ArrayList::new);
  }

  private <T> T route(String op, Supplier<T> local, Supplier<T> extracted, Supplier<T> empty) {
    DomainRoute route = properties.getComment();
    if (route.shadows()) {
      T value = local.get();
      shadow.compareAsync(DOMAIN, op, value, extracted);
      return value;
    }
    if (!route.readsRemote()) {
      return local.get();
    }
    if (route.monolithAuthoritative() && readAfterWrite.writtenInThisRequest(DOMAIN)) {
      return local.get();
    }
    try {
      return extracted.get();
    } catch (CommentServiceException e) {
      return fallback(route, op, local, empty, e);
    }
  }

  private <T> T fallback(
      DomainRoute route,
      String op,
      Supplier<T> local,
      Supplier<T> empty,
      CommentServiceException cause) {
    log.warn(
        "comment-service read failed op={} fallback={} cause={}",
        op,
        route.getFallback(),
        cause.getMessage());
    switch (route.getFallback()) {
      case MONOLITH:
        return local.get();
      case EMPTY:
        return empty.get();
      case FAIL:
      default:
        throw cause;
    }
  }
}

...
Status: **APPROVED — implementation in progress** (approved after Phase 2 completion; Phase 2 = PR #23).
...
#### Canonical internal API (pinned for implementation; mirrors the Phase 1/2 conventions)

Port 8083, DB `article.db`, same `jwt.secret`/HS512 as the monolith. Migrations: `V1__create_article_tables.sql` (`articles`, `tags`, `article_tags` — identical DDL to the monolith, no FKs, no unique constraint on `tags.name`), `V2__seed_tags.sql` (monolith tag + article_tags seed rows only; `articles` stays empty until Phase 4).

| Method / path | Auth | Request | Response |
|---|---|---|---|
| `GET /internal/tags` | none | — | `200 {"tags":["name",...]}` in `select name from tags` order (rowid/insertion order, duplicates included if present) |
| `GET /internal/articles/tags?articleIds=a,b,c` | none | — | `200 {"articleTags":[{"articleId":"a","tagList":["..."]}, ...]}` — one entry per *requested* id (empty `tagList` for ids with no tags); `tagList` in `article_tags` rowid order; empty `articleIds` -> `{"articleTags":[]}` |
| `GET /internal/tags/{name}/article-ids` | none | — | `200 {"articleIds":[...]}` (distinct, `article_tags` rowid order); unknown tag -> `{"articleIds":[]}` |
| `PUT /internal/articles/{articleId}/tags` | `Token <jwt>` (any valid user; authorization stays in the monolith); missing/invalid 401 | `{"tags":[{"id":"<uuid>","name":"..."}]}` — ids are generated by the caller so dual-write produces identical `tags` rows | `200 {"articleId","tagList":[...]}`; idempotent: for each tag, reuse existing row by `name` else insert with the supplied id; insert `(article_id, tag_id)` only if the pair is absent; never deletes relations (matches `MyBatisArticleRepository.createNew`, which only runs on create — article update/delete do not touch tags today) |
| `GET /actuator/health` | none | — | `{"status":"UP"}` |

Errors use the monolith envelope `{"errors":{"body":["..."]}}`. Monolith flags: `extraction.tag.{enabled,read,write,base-url,connect-timeout,read-timeout,fallback}` with the same defaults as Phase 1/2 (`enabled=false`, `read=monolith`, `write=monolith`, `base-url=http://localhost:8083`, `500ms`/`1500ms`, `fallback=monolith`).
